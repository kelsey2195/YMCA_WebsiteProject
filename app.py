...
def program_search():
    ...
    cursor = connection.cursor(prepared=True)
    query = ''' SELECT name_program, start_date, end_date, description, %s, num_total_people, num_signed_up FROM programs '''
    cursor.execute(query, (price_type, ))
    result = cursor.fetchall()
    create_table(result)
    cursor.close()
...
def create_table(result):
    ...
    header = {'Program', 'Start Date', 'End Date', 'Description', 'Price', 'Max Slots', 'Slots Taken Up'}
    table += "  <tr>\n"
